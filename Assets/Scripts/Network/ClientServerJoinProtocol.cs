...
public class ClientServerJoinProtocol : NetworkBehaviour
{
   ...
   [ClientRpc]
    ...
    {
        GameObject.Find("Client").GetComponentInChildren<TMPro.TextMeshProUGUI>().text = "Join";
        
        //Remove the Breed/Gender buttons to avoid changing prefabs with 2nd stage - temporary measure, not needed in future 
        GameObject.Find("Breed").SetActive(false);
        GameObject.Find("Gender").SetActive(false);
    }
    ...
    void LoadLocalNetworkPrefab(string playerPrefab, uint hash, int mode, ulong clientID)
    {
        ...
        else Debug.Log("No need to add " + playerPrefab + " to dictionary as it already exists");    //duplicate network prefabs results in exception error
