...
public class HumanoidNetworkPlayer : NetworkBehaviour
{
    ...
    [SerializeField] Vector3 thirdPOffset = new Vector3(0,0,1.5f);
    [SerializeField] Vector3 firstPOffset = new Vector3(0,0,0.0f);   //BUGFIX - INIT
    ...
    public Dictionary<string, Mesh> FPDict = new Dictionary<string, Mesh>();        //store meshes for all avatar objects in FP mode
    ...
    public override void OnNetworkSpawn()
    {
        ...
        //BUGFIX
        cameraPOffset = myXRRig.transform.rotation * (thirdPToggle.isOn ? thirdPOffset : firstPOffset); //global coordinates
    }
    ...
    void Update()
    {
        ...
        switch (HaCM.m_LeftInputMode)
        {
            case XRInputModalityManager.InputMode.MotionController:
                if (avLeft)
                {
                    ...
                    //avLeft.localPosition = Quaternion.Inverse(myCameraOffset.rotation).normalized * (myXRLC.position - myCameraOffset.position); //NORMALIZE to avoid unwanted rescaling
            ...
            case XRInputModalityManager.InputMode.TrackedHand:
                if (avLeft)
                {
                    ...
                    //avLeft.localPosition = Quaternion.Inverse(myCameraOffset.rotation).normalized * (myXRLH.position - myCameraOffset.position);
        ...
        switch (HaCM.m_RightInputMode)
        {
            case XRInputModalityManager.InputMode.MotionController:
                if (avRight)
                {
                    ...
                    //avRight.localPosition = Quaternion.Inverse(myCameraOffset.rotation).normalized * (myXRRC.position - myCameraOffset.position); //NORMALIZE to avoid unwanted rescaling
            ...
            case XRInputModalityManager.InputMode.TrackedHand:
                if (avRight)
                {
                    ...
                    //avRight.localPosition = Quaternion.Inverse(myCameraOffset.rotation).normalized * (myXRRH.position - myCameraOffset.position);
        ...
        if (avHead)
        {
            //avHead.position = myXRCam.position + avatarHeadPositionOffset;    //not needed
        ...
        if (avBody)
        {
            ...
            //BUGFIX===================THIS CODE NOW GENERICALLY WORKS IN ANY STATE
            ...
            //BUGFIX: add animations when we move the avatar because of head movement
            avatarOldPosition = avBody.position;

            //BUGFIX: added real world horizontal camera offset
            //TWO UX OPTIONS:
            ...
            if (avatarMovesWithRotation) cameraPOffset = avBody.rotation * (thirdPToggle.isOn ? thirdPOffset : firstPOffset);               //Option 1
            else                         cameraPOffset = myXRRig.transform.rotation * (thirdPToggle.isOn ? thirdPOffset : firstPOffset);    //Option 2
            ...
            //BUGFIX, rotate the body with the head horizontal angle - can't use Eulerangles as they are glitchy!
    ...
    [ServerRpc] public void moveMyGrabbedObjectServerRpc(NetworkObjectReference grabbedObj, Vector3 position, Quaternion rotation)
    {
        ...
        if (grabbedObj.TryGet (out NetworkObject netObj))
        {
            ...
            //BUGFIXES - we can remove the below if all is working well
